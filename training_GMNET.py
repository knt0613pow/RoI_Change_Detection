...
for epoch in tqdm(range(0,40)):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        loss1 = criterion(outputs, X_gt)
        # breakpoint()
        match = pygm.hungarian(outputs)
        similar_score = torch.matmul(f1, f2.T)

        loss2 = criterion(match, similar_score)
        loss = loss1+loss2
        loss.backward()
        optimizer.step()
